...
{
	static auto CL_FullUpdate = reinterpret_cast<CL_FullUpdate_t>(PatternSearch("engine.dll", reinterpret_cast<PBYTE>("\xA1\x00\x00\x00\x00\xB9\x00\x00\x00\x00\x56\xFF\x50\x14\x8B\x34\x85"), "x????x????xxxxxxx", NULL, NULL));
	CL_FullUpdate();
}
...
void Menu3D(int x, int y, int w, int h, unsigned long title)
{
	...
	g_pDraw.DrawString(title, true, x + w / 2, y - 7, 255, 255, 255, 255, "Minaye");
...
void CGui::DrawCombobox(int x, int y, const char* var, int num, int max, std::string* structs, int distance, int lenght)
{
	cComboBox[num].Name = const_cast<char*>(structs[g_pCvar.getValue(var)].c_str());
	g_pDraw.FillRGBA(x + distance, y - 2, 75 + lenght, 22, 45, 44, 49, 255);
	g_pDraw.DrawOutlineRect(x + distance, y - 2, 75 + lenght, 23, 110, 110, 110, 255);

	if (MouseOver(x + distance, y - 2, 75 + lenght, 22) && !cComboBox[num].Open)
		g_pDraw.FillRGBA(x + distance, y - 2, 75 + lenght, 22, 200, 200, 200, 50);

	if (cComboBox[num].Active)
	{
		cComboBox[num].iSelect = g_pCvar.getValue(var);

		if (MouseOver(x + distance, y - 2, 75 + lenght, 22) && bLeftClickRelease)
		{
	...
	else
	{
		if (cComboBox[num].Open)
		{
			for (int i = 0; i < max; i++)
			{
				g_pDraw.FillRGBA(x + 2 + distance, y - 1 + (i * 20), 71 + lenght, 20, 45, 44, 49, 255);
				g_pDraw.DrawOutlineRect(x + 2 + distance, y + (i * 20), 71 + lenght, 20, 150, 150, 150, 50);

				if (MouseOver(x + 4 + distance, y + (i * 20), 68 + lenght, 20))
				{
					g_pDraw.FillRGBA(x + 2 + distance, y + 1 + (i * 20), 71 + lenght, 20, 200, 200, 200, 50);
					...
					if (bLeftClickRelease)
					{
						...
						//std::cout << "### Ding ding vi har ændret os! ###" << std::endl;
						//KnifeApplyCallbk();
					}
				}
	...
	if (cComboBox[num].Active == true)
		g_pDraw.DrawString(g_pDraw.font_menutab, false, x + 60 + lenght + distance, y + 2, 200, 200, 200, 255, "+");
}
...
void CGui::DrawMenu()
{
	...
	if (cursor)
	{
		...
		DragMenu("menu_x", "menu_y", w, 48, x1, y1);

		static int iTab = 1;
		ThreeDBorder(g_pCvar.getValue("menu_x"), g_pCvar.getValue("menu_y"), w, h, "Minaye");

		int xspace = 1;

		if (LeftClick(g_pCvar.getValue("menu_x") + 0, g_pCvar.getValue("menu_y") + 22, 108, 25))
			iTab = 1;

		if (LeftClick(g_pCvar.getValue("menu_x") + 108,g_pCvar.getValue("menu_y") + 22, 108, 25))
			iTab = 2;

		if (LeftClick(g_pCvar.getValue("menu_x") + 216, g_pCvar.getValue("menu_y") + 22, 108, 25))
			iTab = 3;

		if (LeftClick(g_pCvar.getValue("menu_x") + 324, g_pCvar.getValue("menu_y") + 22, 108, 25))
			iTab = 4;

		if (LeftClick(g_pCvar.getValue("menu_x") + 432, g_pCvar.getValue("menu_y") + 22, 108, 25) && g_pCvar.getValue("esp_players") == 1)
			iTab = 5;


		if (MouseOver(g_pCvar.getValue("menu_x") + 1, g_pCvar.getValue("menu_y") + 22, 108, 25))
			g_pDraw.FillRGBA(g_pCvar.getValue("menu_x") + 1, g_pCvar.getValue("menu_y") + 22, 108, 25, g_pCvar.getValue("menu_r"), g_pCvar.getValue("menu_g"), g_pCvar.getValue("menu_b"), 100);

		if (MouseOver(g_pCvar.getValue("menu_x") + 108, g_pCvar.getValue("menu_y") + 22, 108, 25))
			g_pDraw.FillRGBA(g_pCvar.getValue("menu_x") + 108, g_pCvar.getValue("menu_y") + 22, 108, 25, g_pCvar.getValue("menu_r"), g_pCvar.getValue("menu_g"), g_pCvar.getValue("menu_b"), 100);

		if (MouseOver(g_pCvar.getValue("menu_x") + 216, g_pCvar.getValue("menu_y") + 22, 108, 25))
			g_pDraw.FillRGBA(g_pCvar.getValue("menu_x") + 216, g_pCvar.getValue("menu_y") + 22, 108, 25, g_pCvar.getValue("menu_r"), g_pCvar.getValue("menu_g"), g_pCvar.getValue("menu_b"), 100);

		if (MouseOver(g_pCvar.getValue("menu_x") + 324, g_pCvar.getValue("menu_y") + 22, 108, 25) && g_pCvar.getValue("esp_players") == 1)
			g_pDraw.FillRGBA(g_pCvar.getValue("menu_x") + 324, g_pCvar.getValue("menu_y") + 22, 108, 25, g_pCvar.getValue("menu_r"), g_pCvar.getValue("menu_g"), g_pCvar.getValue("menu_b"), 100);

		if (MouseOver(g_pCvar.getValue("menu_x") + 432, g_pCvar.getValue("menu_y") + 22, 108, 25))
			g_pDraw.FillRGBA(g_pCvar.getValue("menu_x") + 431, g_pCvar.getValue("menu_y") + 22, 108, 25, g_pCvar.getValue("menu_r"), g_pCvar.getValue("menu_g"), g_pCvar.getValue("menu_b"), 100);

		g_pDraw.DrawString(g_pDraw.font_menuarrows, true, g_pCvar.getValue("menu_x") + 108 / 2 , g_pCvar.getValue("menu_y") +  26, 255, 255, 255, 255, "AIMBOT");
		g_pDraw.DrawString(g_pDraw.font_menuarrows, true, g_pCvar.getValue("menu_x") + 108 / 2 + 108, g_pCvar.getValue("menu_y") +  26, 255, 255, 255, 255, "VISUALS");
		g_pDraw.DrawString(g_pDraw.font_menuarrows, true, g_pCvar.getValue("menu_x") + 108 / 2 + 216, g_pCvar.getValue("menu_y") +  26, 255, 255, 255, 255, "MISC");
		g_pDraw.DrawString(g_pDraw.font_menuarrows, true, g_pCvar.getValue("menu_x") + 108 / 2 + 324, g_pCvar.getValue("menu_y") +  26, 255, 255, 255, 255, "COLOURS");
		if (g_pCvar.getValue("esp_players") == 1)
			g_pDraw.DrawString(g_pDraw.font_menuarrows, true, g_pCvar.getValue("menu_x") + 108 / 2 + 432, g_pCvar.getValue("menu_y") +  26, 255, 255, 255, 255, "PLAYERS");

		xspace = 108;

		if (iTab == 1)
		{
			g_pDraw.FillRGBA(g_pCvar.getValue("menu_x") + xspace - 107, g_pCvar.getValue("menu_y") + 22, 108, 25, g_pCvar.getValue("menu_r"), g_pCvar.getValue("menu_g"), g_pCvar.getValue("menu_b"), 255);
			g_pDraw.DrawString(g_pDraw.font_menuarrows, true, g_pCvar.getValue("menu_x") + 108 / 2, g_pCvar.getValue("menu_y") + 26, 255, 255, 255, 255, "AIMBOT");
			...
			spaceX += 25;
			DrawSlider(g_pCvar.getValue("menu_x") + 30, g_pCvar.getValue("menu_y") + spaceX, 200, 6, "aimbot_fov", 0, 180, 237);
			spaceX += 25;
			DrawSlider(g_pCvar.getValue("menu_x") + 30, g_pCvar.getValue("menu_y") + spaceX, 200, 6, "aimbot_spot", 0, 20, 237);
			spaceX += 25;
			DrawSlider(g_pCvar.getValue("menu_x") + 30, g_pCvar.getValue("menu_y") + spaceX, 200, 6, "aimbot_smooth", 0, 20, 237);
			spaceX += 25;
			GetKeyPressed(g_pCvar.getValue("menu_x") + 30, g_pCvar.getValue("menu_y") + spaceX, 50, 15, 150, 150, 150, 255, "aimbot_key", 430);
			spaceX += 50;
			Checkbox(g_pCvar.getValue("menu_x") + 30, g_pCvar.getValue("menu_y") + spaceX, "aimbot_silent", 450, g_pDraw.font_menuoptions);
			spaceX += 25;
			Checkbox(g_pCvar.getValue("menu_x") + 30, g_pCvar.getValue("menu_y") + spaceX, "aimbot_norecoil", 450, g_pDraw.font_menuoptions);
			spaceX += 25;
			Checkbox(g_pCvar.getValue("menu_x") + 30, g_pCvar.getValue("menu_y") + spaceX, "aimbot_autofire", 450, g_pDraw.font_menuoptions);
			spaceX += 25;
			Checkbox(g_pCvar.getValue("menu_x") + 30, g_pCvar.getValue("menu_y") + spaceX, "aimbot_antiaim", 450, g_pDraw.font_menuoptions);
			spaceX += 25;
			/*
		...
		if (iTab == 2)
		{
			g_pDraw.FillRGBA(g_pCvar.getValue("menu_x") + xspace - 108, g_pCvar.getValue("menu_y") + 22, 108, 25, g_pCvar.getValue("menu_r"), g_pCvar.getValue("menu_g"), g_pCvar.getValue("menu_b"), 255);
			g_pDraw.DrawString(g_pDraw.font_menuarrows, true, g_pCvar.getValue("menu_x") + 108 / 2 + 108, g_pCvar.getValue("menu_y") +  26, 255, 255, 255, 255, "VISUALS");
			...
			spaceX += 25;
			Checkbox(g_pCvar.getValue("menu_x") + 30, g_pCvar.getValue("menu_y") + spaceX, "esp_skeleton", 450, g_pDraw.font_menuoptions);
			spaceX += 25;
			...
			spaceX += 25;
			Checkbox(g_pCvar.getValue("menu_x") + 30, g_pCvar.getValue("menu_y") + spaceX, "esp_money", 450, g_pDraw.font_menuoptions);
		}
		...
		if (iTab == 3)
		{
			...
			spaceX += 25;
			Checkbox(g_pCvar.getValue("menu_x") + 30, g_pCvar.getValue("menu_y") + spaceX, "misc_skinchanger", 450, g_pDraw.font_menuoptions);
			spaceX += 50;
			DrawSlider(g_pCvar.getValue("menu_x") + 30, g_pCvar.getValue("menu_y") + spaceX, 200, 6, "menu_r", 0, 250, 237);
			spaceX += 25;
			DrawSlider(g_pCvar.getValue("menu_x") + 30, g_pCvar.getValue("menu_y") + spaceX, 200, 6, "menu_g", 0, 250, 237);
			spaceX += 25;
			DrawSlider(g_pCvar.getValue("menu_x") + 30, g_pCvar.getValue("menu_y") + spaceX, 200, 6, "menu_b", 0, 250, 237);
			spaceX += 50;
			g_pDraw.DrawString(g_pDraw.font_menuoptions, false, g_pCvar.getValue("menu_x") + 30, g_pCvar.getValue("menu_y") + spaceX + (float)1.5, 150, 150, 150, 255, "Knife:");
			g_pDraw.DrawString(g_pDraw.font_menuoptions, false, g_pCvar.getValue("menu_x") + 30 + 120, g_pCvar.getValue("menu_y") + spaceX + (float)1.5, 150, 150, 150, 255, "T:");
			g_pDraw.DrawString(g_pDraw.font_menuoptions, false, g_pCvar.getValue("menu_x") + 30 + 305, g_pCvar.getValue("menu_y") + spaceX + (float)1.5, 150, 150, 150, 255, "CT:");
			spaceX += 25;
			DrawCombobox(g_pCvar.getValue("menu_x") + 30, g_pCvar.getValue("menu_y") + spaceX, "misc_skinchanger_knife", 1, 6, knives, 0, 35);
			DrawCombobox(g_pCvar.getValue("menu_x") + 30, g_pCvar.getValue("menu_y") + spaceX, "misc_skinchanger_knifeskin_t", 2, 23, knifeskins, 120, 100);
			DrawCombobox(g_pCvar.getValue("menu_x") + 30, g_pCvar.getValue("menu_y") + spaceX, "misc_skinchanger_knifeskin_ct", 3, 23, knifeskins, 305, 100);
		}
		...
		if (iTab == 4)
		{
			...
			spaceX += 50;
			DrawSlider(g_pCvar.getValue("menu_x") + 30, g_pCvar.getValue("menu_y") + spaceX, 200, 6, "esp_text_color_r", 0, 250, 237);
			spaceX += 25;
		...
		{
			g_pDraw.FillRGBA(g_pCvar.getValue("menu_x") + xspace - 109, g_pCvar.getValue("menu_y") + 22, 108, 25, g_pCvar.getValue("menu_r"), g_pCvar.getValue("menu_g"), g_pCvar.getValue("menu_b"), 255);
			g_pDraw.DrawString(g_pDraw.font_menuarrows, true, g_pCvar.getValue("menu_x") + 108 / 2 + 432, g_pCvar.getValue("menu_y") +  26, 255, 255, 255, 255, "PLAYERS");
