...
public:
	...
	// return true if this vtor is (0,0) within tolerance
	bool IsZero(float tolerance = 0.01f) const
	{
		return (x > -tolerance && x < tolerance &&
			y > -tolerance && y < tolerance);
	...
	// Cross product between two vtors.
...
const Vector2D v2_origin(0, 0);
const Vector2D v2_invalid(FLT_MAX, FLT_MAX);
...
// Linearly interpolate between two vtors
...
//-----------------------------------------------------------------------------
// Computes the closest point to vTarget no farther than flMaxDist from vStart
//-----------------------------------------------------------------------------
inline void ComputeClosestPoint2D(const Vector2D& vStart, float flMaxDist, const Vector2D& vTarget, Vector2D *pResult)
{
	Vector2D vDelta;
	Vector2DSubtract(vTarget, vStart, vDelta);
	float flDistSqr = vDelta.LengthSqr();
	...
	{
		vDelta /= FastSqrt(flDistSqr);
		Vector2DMA(vStart, flMaxDist, vDelta, *pResult);
	}
}
